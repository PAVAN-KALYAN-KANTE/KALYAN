...
ll get_index(ll y,vector<ll>vt){
    /*ll index=-1;
 for(int round=y; round < v1.size(); round++)
    ...
    return index;*/
  std::vector<ll>::iterator upper1 = std::upper_bound(vt.begin(), vt.end(), y);
  ll d=upper1-vt.begin();
  return d;
}
...
int main(){
    ...
    for(ll i=0;i<t;i++){
        ...
        vector<ll>v2(m);
        vector<ll>vo,vt;
        for(ll j=0;j<n;j++){cin>>v1[j];if(v1[j]==1){vo.push_back(j);}else if(v1[j]==2){vt.push_back(j);}}
            ...
            if(v1[k]!=0&&k!=0){count=0;}
             ...
             // cout<<x<<" x "<<b<<" b"<<endl;
              ...
              /* for(auto a:v3){
               ...
               if(x!=-1){res1=x-k;cout<<res1<<"????"<<endl;}
               if(b!=-1){res2=k-b;cout<<res2<<"????"<<endl;}
               count=min(res1,res2);
              }
